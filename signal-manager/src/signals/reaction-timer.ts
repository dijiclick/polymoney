...
export const reactionTimerSignal: SignalFunction = (event, changedKeys, source) => {
  ...
  const TRACKED_SOURCES = ['polymarket', 'onexbet', 'kambi', 'thesports', 'sofascore', 'pinnacle', 'bet365'];
...
function finalizePendingGoal(goalKey: string, pending: PendingGoal): void {
  ...
  log.debug(`⏱ STABILIZED ${entry.match} | ${entry.scoreBefore} → ${entry.scoreAfter} | by ${entry.detectedBy} | ${trajEntries.length} trajectories`);
  if (pmTrajs.length > 0) {
    log.debug(`  PM: ${pmTrajs.map(fmtTraj).join(' | ')}`);
  }
  if (xbTrajs.length > 0) {
    log.debug(`  1xBet: ${xbTrajs.map(fmtTraj).join(' | ')}`);
  }
}
